...
function useCounter(initialValue = 0, options) {
	...
	const [count, setCount] = (0, import_react.useState)(clamp$1(initialValue, min, max));
	return [count, {
		increment: (0, import_react.useCallback)(() => setCount((current) => clamp$1(current + step, min, max)), [
			min,
			max,
			step
		]),
		decrement: (0, import_react.useCallback)(() => setCount((current) => clamp$1(current - step, min, max)), [
			min,
			max,
			step
		]),
		set: (0, import_react.useCallback)((value) => setCount(clamp$1(value, min, max)), [min, max]),
		reset: (0, import_react.useCallback)(() => setCount(clamp$1(initialValue, min, max)), [
			initialValue,
			min,
			max
		])
...
function clampUseMovePosition(position) {
	return {
		x: clamp$1(position.x, 0, 1),
		y: clamp$1(position.y, 0, 1)
	};
}
				...
				frame.current = requestAnimationFrame(() => {
					if (mounted.current && node) {
						...
						if (rect.width && rect.height) {
							const _x = clamp$1((x - rect.left) / rect.width, 0, 1);
							onChange({
								x: dir === "ltr" ? _x : 1 - _x,
								y: clamp$1((y - rect.top) / rect.height, 0, 1)
							});
						}
					}
				});
...
function normalizeRadialValue(degree, step) {
	const clamped = clamp$1(degree, 0, 360);
...
//#endregion
export { useHash as $, useMutationObserverTarget as A, useFocusTrap as At, useFocusWithin as B, useMediaQuery as Bt, useThrottledState as C, useLocalStorage as Ct, useStateHistory as D, useIdle as Dt, useMap as E, useInterval as Et, useFavicon as F, useIsomorphicEffect as Ft, useOs as G, upperFirst as Gt, useEventListener as H, useClickOutside as Ht, usePrevious as I, useDebouncedValue as It, useLogger as J, randomId as Jt, useValidatedState as K, shallowEqual as Kt, useTextSelection as L, useDebouncedState as Lt, useEyeDropper as M, useDidUpdate as Mt, useHeadroom as N, useDocumentVisibility as Nt, useMounted as O, useId$1 as Ot, useScrollDirection as P, useDocumentTitle as Pt, getHotkeyHandler as Q, useTimeout as R, useCounter as Rt, useThrottledValue as S, readLocalStorageValue as St, useSet as T, useListState as Tt, useInputState as U, useDebouncedCallback as Ut, useDisclosure as V, useClipboard as Vt, useSetState as W, useCallbackRef as Wt, useFullscreenElement as X, clamp$1 as Xt, useFullscreenDocument as Y, lowerFirst as Yt, useHotkeys as Z, normalizeRadialValue as _, assignRef as _t, generatePattern as a, useElementSize as at, useOrientation as b, readSessionStorageValue as bt, useMask as c, useReducedMotion as ct, useFloatingWindow as d, usePagination as dt, useIntersection as et, useSelection as f, useUncontrolled as ft, useScrollSpy as g, useMousePosition as gt, useScroller as h, useMouse as ht, formatMask as i, useShallowEffect as it, useInViewport as j, useFocusReturn as jt, useMutationObserver as k, useForceUpdate as kt, useHorizontalCollapse as l, usePageLeave as lt, useFileDialog as m, useMove as mt, useDrag as n, useViewportSize as nt, isMaskComplete as o, useResizeObserver as ot, useLongPress as p, clampUseMovePosition as pt, useHover as q, range$1 as qt, useRovingIndex as r, useToggle as rt, unformatMask as s, useScrollIntoView as st, useSplitter as t, useWindowScroll as tt, useCollapse as u, useQueue as ut, useRadialMove as v, mergeRefs as vt, useThrottledCallback as w, useWindowEvent as wt, useIsFirstRender as x, useSessionStorage as xt, useFetch as y, useMergedRef as yt, useNetwork as z, useColorScheme as zt };

//# sourceMappingURL=esm-0Muzxjuc.js.map